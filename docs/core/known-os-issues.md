...
Each entry includes:
- affected platform/framework
- reproducible symptom pattern
- how to identify it early
- mitigation/workaround strategy
...
- Platform/OS range: iOS/iPadOS (reported Jan 2026)
- Framework: SwiftUI
...
- Platform/OS range: iOS (reported across major versions)
...
- Mitigation/workaround:
  - avoid text baked into images where practical
  - convert critical text-in-image content to true text views
  - regression-test image-heavy surfaces with VoiceOver enabled
- Sources:
...
- Platform/OS range: iOS/iPadOS (reported)
- Framework: SwiftUI
...
- Mitigation/workaround:
  - verify role expectations on every target OS version
...
- Platform/OS range: iOS historical regression window
- Framework: SwiftUI
...
- Platform/OS range: iOS/tvOS (reported pattern)
...
No visionOS-specific app-level regression entries are recorded in this catalog yet.
